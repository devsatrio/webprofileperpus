...
export default function KategoriGaleriPage() {
    const [data, setData] = useState<KategoriGaleri[]>([]);
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            setError("");
            const result = await kategoriGaleriService.getAll();
            setData(result);
        } catch (err: any) {
    ...
    const handleDelete = async (id: number, nama: string) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    await kategoriGaleriService.delete(id);
    ...
    return (
        <div>
            <div className="space-y-6">
                <ComponentCard title="Kategori Galeri">
                    <div className="flex flex-col gap-4 mb-5">
                        <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
                            <Link href="/admin/kategori-galeri/create">
                                            ...
                                            {paginatedData.map((item, key) => (
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400 text-center space-x-1">
                                                        <Link href={`/admin/kategori-galeri/${item.id}/edit`} title="Edit">
