...
export default function SliderPage() {
    ...
    const handleDelete = async (id: number, heading: string, imageUrl: string) => {
        Swal.fire({
            title: "Hapus Data?",
            text: `Slider "${heading}" akan dihapus permanen!`,
            icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    // Hapus image dari storage terlebih dahulu
                    if (imageUrl) {
                        await sliderService.deleteImage(imageUrl);
                    }
    ...
    const filteredData = data.filter((item) =>
        ...
        item.sub_heading.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    return (
        <div>
            <div className="space-y-6">
                <ComponentCard title="Slider Banner">
                    <div className="flex flex-col gap-4 mb-5">
                        <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
                            ...
                            <input
                                type="text"
                                placeholder="Cari heading atau sub heading..."
                                value={searchQuery}
                            ...
                            <div className="flex items-center justify-center py-12">
                                <div className="text-center">
                                    <p className="text-gray-500 dark:text-gray-400">
                                        {searchQuery ? "Tidak ada data yang cocok" : "Belum ada data slider"}
                                    </p>
                                </div>
                            </div>
                        ) : (
                                    ...
                                    <Table>
                                        ...
                                        <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                                            <TableRow>
                                                ...
                                                <TableCell
                                                    isHeader
                                                    className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400 w-32"
                                                >
                                                    Image
                                                </TableCell>
                                                <TableCell
                                            ...
                                            {paginatedData.map((item, key) => (
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell className="px-4 py-3 text-center">
                                                        {item.image_url ? (
                                                            <div className="relative w-24 h-14 mx-auto rounded overflow-hidden">
                                                                <Image
                                                                    src={item.image_url}
                                                                    alt={item.heading}
                                                                    fill
                                                            ...
                                                            <div className="w-24 h-14 mx-auto rounded bg-gray-200 dark:bg-gray-700 flex items-center justify-center">
                                                    ...
                                                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400 text-center space-x-1">
                                                        <Link href={`/admin/slider/${item.id}/edit`} title="Edit">
                                                            <Button size="sm" variant="success">
                                                                <PencilIcon />
                                                            </Button>
                                                        </Link>
                                                        <Button size="sm" variant="danger" onClick={() => handleDelete(item.id, item.heading, item.image_url)}>
                                                            <TrashBinIcon />
                                                        </Button>
                                                    </TableCell>
                                                </TableRow>
