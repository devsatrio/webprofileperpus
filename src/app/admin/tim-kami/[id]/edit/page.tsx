...
export default function EditPage() {
    ...
    const [data, setData] = useState<OurTeam | null>(null);
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                setError("");
                const result = await ourTeamService.getById(id);
                setData(result);
            } catch (err: any) {
    ...
    const handleSubmit = async (formData: Omit<OurTeam, "id">, imageFile: File | null) => {
        try {
            ...
            if (imageFile) {
                // Hapus image lama
                if (data?.image) {
                    await ourTeamService.deleteImage(data.image);
                }
                imageUrl = await ourTeamService.uploadImage(imageFile);
            }

            await ourTeamService.update(id, {
                ...formData,
                image: imageUrl,
            });
    ...
    if (loading) {
        return (
            <ComponentCard title="Edit Tim Kami">
    ...
    return (
        <ComponentCard title="Edit Tim Kami">
            <div className="space-y-6">
                <TimKamiForm initialData={data} onSubmit={handleSubmit} />
            </div>
        </ComponentCard>
    );
}
