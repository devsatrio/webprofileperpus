...
export default function TimKamiPage() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            setError("");
            const result = await ourTeamService.getAll();
            setData(result);
        } catch (err: any) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (item?.image) {
                        await ourTeamService.deleteImage(item.image);
                    }
    ...
    return (
        <div>
            <div className="space-y-6">
                <ComponentCard title="Tim Kami">
                    <div className="flex flex-col gap-4 mb-5">
                        <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
                            <Link href="/admin/tim-kami/create">
                                            ...
                                            {paginatedData.map((item, key) => (
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell className="px-4 py-3 text-center">
                                                        {item.image ? (
                                                            <div className="relative w-12 h-12 rounded-full overflow-hidden border border-gray-300 dark:border-gray-600 mx-auto">
                                                                <Image
                                                                    src={item.image}
                                                                    alt={item.nama || "Foto"}
                                                                    fill
                                                    ...
                                                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400 text-center space-x-1">
                                                        <Link href={`/admin/tim-kami/${item.id}/edit`} title="Edit">
