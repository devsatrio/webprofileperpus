...
export default function GaleriPage() {
  const [data, setData] = useState<GaleriWithKategori[]>([]);
  ...
  const handleDelete = async (id: number, keterangan: string) => {
    Swal.fire({
      title: "Hapus Data?",
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (item?.image_url) {
            await galeriService.deleteImage(item.image_url);
          }
  ...
  const filteredData = data.filter(
    (item) =>
      ...
      item.kategori_galeri?.nama.toLowerCase().includes(searchQuery.toLowerCase())
  );
                        ...
                        <TableRow key={item.id}>
                          ...
                          <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400 text-center space-x-1">
                            ...
                            <Button
                              ...
                              variant="danger"
                              onClick={() =>
                                handleDelete(item.id, item.keterangan)
                              }
                            >
                              <TrashBinIcon />
                            </Button>
