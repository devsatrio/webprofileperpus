...
import { Artikel, artikelService } from "@/services/artikel";
...
export default function ArtikelPage() {
    ...
    const handleDelete = async (id: number, judul: string) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    // Get artikel data to delete image if exists
                    const artikel = data.find(item => item.id === id);
                    if (artikel?.image) {
                        try {
                            await artikelService.deleteImage(artikel.image);
                        } catch (err) {
    ...
    const filteredData = data.filter((item) =>
        ...
        item.kategori_artikel?.nama?.toLowerCase().includes(searchQuery.toLowerCase())
    );
                                    ...
                                    <Table>
                                        ...
                                        <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                                            <TableRow>
                                                ...
                                                <TableCell
                                                    isHeader
                                                    className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400 w-20"
                                                >
                                                    Gambar
                                                </TableCell>
                                                <TableCell
                                                ...
                                                <TableRow key={item.id}>
                                                    ...
                                                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400 text-center">
                                                        {item.image ? (
                                                            ...
                                                            <div className="w-16 h-12 bg-gray-200 dark:bg-gray-700 rounded mx-auto flex items-center justify-center">
                                                    ...
                                                    <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400 text-center space-x-1">
                                                        <Link href={`/admin/artikel/${item.id}/edit`} title="Edit">
                                                            <Button size="sm" variant="success">
                                                                <PencilIcon />
                                                            </Button>
                                                        </Link>
                                                        <Button size="sm" variant="danger" onClick={() => handleDelete(item.id, item.judul)}>
                                                            <TrashBinIcon />
                                                        </Button>
                                                    </TableCell>
                                                </TableRow>
